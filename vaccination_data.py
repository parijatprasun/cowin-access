...
def main():
    ...
    print(f'India vaccinated {single_so_far:,} so far with only single dose')
    print(f'India need {balance_population:,} in {duration} days @ {rvr:,} per day')
    print(f'Current vaccination rate (last {block} days average): {cvr:,}')
    print(f'At this rate India may get only {max_possible:,} and will trail by {trail:,} and may reach target by {new_deadline}')
